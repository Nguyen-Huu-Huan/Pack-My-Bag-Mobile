...
const YourClosetBody = ({ navigation, item_data_list }) => {
	...
	const [allAvailablePositions, setAllAvailablePositions] = useState([1, 2, 3, 4, 5, 6, 7, 8]);
		...
		if (item_data_list) {
			item_data_list.forEach((item) => {
				if (allAvailablePositions.includes(item.closet_position_index)) {
					setAllAvailablePositions(allAvailablePositions.filter((position) => position !== item.closet_position_index));
				}
			});
		}
	...
	return (
		<View style={styles.wrapper}>
			{isLoadingRemoveData ? <ActivityIndicator size="large" color="black" /> : <></>}
			<View style={{ display: "flex", flexDirection: "row" }}>
				...
				>
					<TouchableOpacity onPress={() => navigation.push("ItemInfo", { item_data: [], available_closet_index: allAvailablePositions })}>
						<Text>
							<Ionicons style={styles.icon} color="#222160" name="add-circle-sharp" size={40} />
						</Text>
					</TouchableOpacity>
				</View>
			</View>
			{itemDataList.length > 0 && (
				<ScrollView>
					{itemDataList.map((item_data, index) => {
						return <YourClosetItem key={index} navigation={navigation} item_index={index} item_data={item_data} remove_item={removeItem} />;
					})}
				</ScrollView>
			)}
		</View>
