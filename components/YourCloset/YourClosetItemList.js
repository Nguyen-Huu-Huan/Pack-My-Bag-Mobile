	...
	return (
		<View style={styles.wrapper}>
			<View style={{ display: "flex", flexDirection: "row" }}>
				...
				>
					<TouchableOpacity onPress={() => navigation.push("ItemInfo", { item_data: [] })}>
						<Text>
			...
			{itemDataList.map((item_data, index) => {
				return <YourClosetItem key={index} navigation={navigation} item_index={index} item_data={item_data} remove_item={removeItem} onClick={() => navigation.push("ItemInfo", { item_data: item_data })} />;
			})}
		</View>
	);
};
