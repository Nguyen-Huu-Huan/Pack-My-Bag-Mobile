...
const HomeBody = ({ navigation }) => {
  ...
  const connectToArduino = async () => {
    ...
    await closetItems.forEach(async (item) => {
      try {
        if (
          item.item_location.includes(choice) ||
          item.item_location[0] === "All"
        ) {
          ...
          match_weather_type.forEach((type) => {
            try {
              if (
                item.weather_type.includes(type) ||
                item.weather_type[0] === "All"
              ) {
    ...
    const result = await ClosetController.positionsToGet(
      closetIndexStringValues
    );
