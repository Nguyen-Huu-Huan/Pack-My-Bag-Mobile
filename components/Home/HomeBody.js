...
const HomeBody = ({ navigation }) => {
	...
	const loadCurrentWeather = async () => {
		...
		const response = await fetch(`${url}appid=${openWeatherKey}&lat=${location.coords.latitude}&lon=${location.coords.longitude}`);
	...
	const connectToArduino = async () => {
		...
		sortClosetIndex = [];
		itemDataResult.forEach(async (item) => {
			sortClosetIndex.push(item.closet_position_index);
		});
		sortClosetIndex.sort((a, b) => a - b);
		closetIndexStringValues = "";
		sortClosetIndex.forEach(async (position, index) => {
			if (index === sortClosetIndex.length - 1) {
				closetIndexStringValues += position;
			} else {
				closetIndexStringValues += position + ",";
			}
		});
		const result = await ClosetController.positionsToGet(closetIndexStringValues);
	...
	const renderItem = (item) => {
		return (
			<View style={styles.item}>
				...
				{item.label === choice && <AntDesign color="red" name={item.icon ? item.icon : "checkcircle"} size={20} />}
			</View>
		);
	};
	return (
		<View style={{ position: "relative" }}>
			...
			>
				...
				>
					<FontAwesome5 style={{ marginHorizontal: 10 }} color="black" name="hand-point-right" size={20} />
				...
				<Dropdown
					...
					data={locations}
					maxHeight={300}
					placeholder={choice ? "Place : " + choice : "Choose the avaiable place"}
					onChange={(item) => {
