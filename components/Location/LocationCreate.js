...
import { View, Text, Dimensions, Image, TouchableOpacity, ScrollView, StyleSheet, Button } from "react-native";
...
const LocationCreate = ({ navigation }) => {
	...
	const handleSubmitLocation = async () => {
		if (locationName && locationType && region !== currentPosition) {
			...
			LocationController.createLocation(newLocationData) ? navigation.push("Location", newLocationData) : setIsError(true);
		} else {
			setIsError(true);
		}
		setShowPopover(true);
		...
		<View style={{ flex: 1, position: "relative" }}>
			...
			>
				...
				<TouchableOpacity onPress={handleSubmitLocation}>
					<Text>
						<MaterialIcons style={styles.icon} color="black" name="add-business" size={40} />
					</Text>
				</TouchableOpacity>
				<Popover isVisible={showPopover} onRequestClose={() => setShowPopover(false)}>
					...
					>
						...
						<Text style={{ fontSize: 30, marginVertical: 10, fontFamily: "font1" }}>{isError ? "Error!" : "Confirm!"}</Text>
						<Text style={{ fontSize: 16, color: "#989898" }}>{isError ? "You must fill all the details" : "Your customize location has been added"}</Text>
						<TouchableOpacity
							style={{
								marginTop: 30,
								borderRadius: 5,
